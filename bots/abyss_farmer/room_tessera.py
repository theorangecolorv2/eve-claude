"""
Модуль для прохождения комнаты со Strikegrip Tessera.

Особенности:
- ПКМ по Tessera → наведение на "выйти на орбиту" → клик "15 км"
- Выпуск дронов
- Атака с отслеживанием здоровья дронов
- При падении HP дронов - возврат, пауза 3с, повторный выпуск
- После убийства Tessera - аппроч к контейнеру, ожидание <= 35км, атака
- Лут контейнера
- Зачистка остальных врагов (если есть)
- Возврат дронов и прыжок через ворота
"""
import logging
import time
import random
...
def tessera_room(sanderling: SanderlingService, timeout: float = 300.0) -> bool:
    """
    Пройти комнату со Strikegrip Tessera.
    ...
    """
    logger.info("=== НАЧАЛО ПРОХОЖДЕНИЯ КОМНАТЫ (TESSERA) ===")
    ...
    # 4. Проверить наличие Strikegrip Tessera
    logger.info("Шаг 4: Поиск Strikegrip Tessera...")
    tessera = _find_tessera(sanderling)
    if not tessera:
        logger.error("Strikegrip Tessera не найден")
        return False
    
    logger.info(f"Tessera найден: {tessera.name}")
    
    # 5. ПКМ по Tessera → наведение на орбиту → клик 15 км
    logger.info("Шаг 5: Орбита Tessera 15 км...")
    if not _orbit_15km(sanderling, tessera):
        logger.error("Не удалось выйти на орбиту 15 км")
        return False
    ...
    # 7. Ждать пока Tessera <= 35 км
    logger.info("Шаг 7: Ожидание Tessera в радиусе <= 35 км...")
    if not _wait_target_in_range(sanderling, "Tessera", max_distance_km=35.0, timeout=60.0):
        logger.warning("Tessera не подлетел в радиус 35 км")
    
    # 8. Залочить Tessera
    logger.info("Шаг 8: Лок Tessera...")
    if not _lock_target(sanderling, tessera):
        logger.error("Не удалось залочить Tessera")
        return False
    
    # 9. Атаковать с отслеживанием здоровья дронов
    logger.info("Шаг 9: Атака Tessera с отслеживанием дронов...")
    if not _kill_with_drone_monitoring(sanderling, target_name="Tessera"):
        logger.error("Не удалось убить Tessera")
        return False
    
    logger.info("Tessera убит!")
    ...
    if is_wreck:
        ...
        # 16. Лут контейнера (таймаут 30 секунд)
    ...
    # 19. Зачистка остальных врагов (если есть)
    logger.info("Шаг 19: Зачистка оставшихся врагов...")
...
# ============================================================================
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (импортируем из room_new где возможно)
# ============================================================================
...
def _find_tessera(sanderling: SanderlingService) -> Optional[OverviewEntry]:
    """
    Найти Strikegrip Tessera в overview.
...
def _orbit_15km(sanderling: SanderlingService, target: OverviewEntry) -> bool:
    """
    ПКМ по цели → наведение на "выйти на орбиту" → клик "15 км".
    ...
    for attempt in range(5):
        time.sleep(0.7)  # Пауза перед проверкой
        
        state = sanderling.get_state()
        if not state or not state.context_menu:
            continue
        
        # Проверяем есть ли пункты с радиусами (например "15 км")
    ...
    # Шаг 6: Найти пункт "15 км"
    orbit_15km_item = None
    for item in state.context_menu.items:
        text = item.text.strip()
        if text == "15 км" or text == "15км":
            orbit_15km_item = item
            break
    
    if not orbit_15km_item:
        logger.error("Пункт '15 км' не найден")
        logger.error(f"Доступные пункты: {[item.text for item in state.context_menu.items]}")
        return False
    
    logger.debug(f"Найден пункт '15 км' @ {orbit_15km_item.center}")
    
    # Шаг 7: Кликнуть на "15 км"
    logger.debug("Клик на '15 км'...")
    click(orbit_15km_item.center[0], orbit_15km_item.center[1], duration=0.18)
    random_delay(0.5, 0.8)
    
    logger.info("Орбита 15 км установлена")
    return True
...
def _wait_target_in_range(
    ...
    Args:
        sanderling: Сервис Sanderling
        target_keyword: Ключевое слово для поиска цели (например, "Tessera" или "cache")
...
def _lock_target(sanderling: SanderlingService, target: OverviewEntry) -> bool:
    ...
    random_delay(2.5, 3.0)
    
    return True


def _kill_with_drone_monitoring(sanderling: SanderlingService, target_name: str) -> bool:
    """
    Убить цель с отслеживанием здоровья дронов.
    
    Если здоровье дронов начинает падать:
    1. Возврат дронов (Shift+R)
    2. Ожидание возврата
    3. Пауза 3 секунды
    4. Повторный выпуск
    5. Продолжение атаки
    ...
    """
    # Шаг 1: Начать атаку
    logger.info("Начинаю атаку (1 + F)...")
    press_key('1')
    random_delay(0.3, 0.5)
    press_key('f')
    random_delay(0.5, 0.8)
    
    # Шаг 2: Запомнить начальное здоровье дронов
    initial_health = _get_drones_health(sanderling)
    if not initial_health:
        logger.warning("Не удалось получить начальное здоровье дронов")
        initial_health = {}
    
    logger.debug(f"Начальное здоровье дронов: {initial_health}")
    ...
    while time.time() - start < kill_timeout:
        ...
        if current_health and initial_health:
            health_dropped = _check_health_drop(initial_health, current_health)
            
            if health_dropped:
                logger.warning("Здоровье дронов падает! Возвращаю дронов...")
                
                # Возврат дронов
                if not _recall_drones(sanderling):
                    logger.error("Не удалось вернуть дронов")
                    return False
                ...
                # Повторный выпуск
                logger.info("Повторный выпуск дронов...")
                _launch_drones()
                random_delay(1.5, 2.0)
                
                # Ждем пока дроны выпустятся
                if not _wait_drones_launched(sanderling, timeout=10.0):
                    logger.error("Дроны не выпустились")
                    return False
                
                # Продолжаем атаку
                logger.info("Продолжаю атаку...")
                press_key('f')
                random_delay(0.5, 0.8)
...
def _get_drones_health(sanderling: SanderlingService) -> Dict[str, Dict[str, float]]:
    """
    Получить текущее здоровье всех дронов в космосе.
    
    Args:
        sanderling: Сервис Sanderling
        
    Returns:
        Словарь {имя_дрона: {'shield': float, 'armor': float, 'hull': float}}
    """
    state = sanderling.get_state()
    ...
    health = {}
    for i, drone in enumerate(state.drones.drones_in_space):
        # Используем индекс как ключ, т.к. имена могут повторяться
        key = f"{drone.name}_{i}"
        health[key] = {
            'shield': drone.shield,
            'armor': drone.armor,
            'hull': drone.hull
...
def _check_health_drop(initial: Dict, current: Dict) -> bool:
    """
    Проверить упало ли здоровье дронов.
    
    Args:
        initial: Начальное здоровье
        current: Текущее здоровье
        
    Returns:
        True если здоровье упало
    """
    for key in initial:
        ...
        if curr_hp['shield'] < init_hp['shield'] - 0.01:  # Порог 1%
            logger.debug(f"{key}: shield упал с {init_hp['shield']:.2f} до {curr_hp['shield']:.2f}")
            return True
        
        if curr_hp['armor'] < init_hp['armor'] - 0.01:
            logger.debug(f"{key}: armor упал с {init_hp['armor']:.2f} до {curr_hp['armor']:.2f}")
            return True
        
        if curr_hp['hull'] < init_hp['hull'] - 0.01:
            logger.debug(f"{key}: hull упал с {init_hp['hull']:.2f} до {curr_hp['hull']:.2f}")
            return True
    
    return False


def _wait_drones_launched(sanderling: SanderlingService, timeout: float) -> bool:
    """
    Ждать пока дроны выпустятся (in_space_count > 0).
    ...
    Returns:
        True если дроны выпустились
    """
    start = time.time()
    
    while time.time() - start < timeout:
        state = sanderling.get_state()
        if state and state.drones and state.drones.in_space_count > 0:
            logger.info(f"Дроны выпущены: {state.drones.in_space_count}")
            return True
        
        time.sleep(0.5)
    
    logger.warning(f"Таймаут {timeout}с ожидания выпуска дронов")
    return False


def _loot_wreck(sanderling: SanderlingService) -> bool:
    """
    Залутать врек контейнера (как в default_room).
    ...
    for attempt in range(25):  # 30 секунд для Tessera
        random_delay(1.2, 1.5)
    ...
    logger.warning("Не удалось найти кнопку 'Взять все' за 30 секунд, продолжаю...")
